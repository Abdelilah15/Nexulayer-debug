...
const NetworkAvatar = ({ name, iconUrl, className = "" }: { name: string, iconUrl?: string, className?: string }) => {
    ...
    if (hasError || !iconUrl) {
        return (
            <img src="/globe.svg" alt="Default Network" className={`object-cover bg-background p-[2px] ${className}`} />
        );
    }
        ...
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="flex bg-background rounded-xl border border-card p-1 overflow-hidden">
                    <button onClick={() => setSelectedAsset('Tokens')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${selectedAsset === 'Tokens' ? "bg-[#2b7fff] text-white" : "text-secondary hover:text-foreground"}`}>Tokens</button>
                    <button onClick={() => setSelectedAsset('DeFi')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${selectedAsset === 'DeFi' ? "bg-[#2b7fff] text-white" : "text-secondary hover:text-foreground"}`}>DeFi</button>
                    <button onClick={() => setSelectedAsset('NFTs')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${selectedAsset === 'NFTs' ? "bg-[#2b7fff] text-white" : "text-secondary hover:text-foreground"}`}>NFTs</button>
                </div>
                    ...
                    <div className="space-y-1">
                        {filteredAssets.length > 0 ? (
                            filteredAssets.map((asset, index) => {
                                ...
                                return (
                                    <div key={`${asset.id}-${index}`} className="grid grid-cols-3 items-center px-4 py-3 border-b border-card rounded-xl hover:bar-button-hover transition-colors">
                                        <div className="flex items-center gap-3">
                                            <div className="relative shrink-0">
                                                <TokenIcon asset={asset} />
                                                <NetworkAvatar name={asset.chainName} iconUrl={asset.chainIcon} className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full" />
                                            </div>
                    ...
                    <div className="space-y-1">
                        {filteredAssets.length > 0 ? (
                            filteredAssets.map((asset, index) => {
                                ...
                                return (
                                    <div key={`${asset.id}-${index}`} className="grid grid-cols-3 items-center px-4 py-3 rounded-xl border-b border-card hover:bar-button-hover transition-colors">
                                        <div className="flex items-center gap-3">
                                            <div className="relative shrink-0">
                                                <TokenIcon asset={asset} />
                                                <NetworkAvatar name={asset.chainName} iconUrl={asset.chainIcon} className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full" />
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    ...
                                                    <span className={`text-[9px] uppercase font-bold px-1.5 py-0.5 rounded ${
                                                        asset.assetType === 'staking' ? "bg-emerald-500/10 text-emerald-500" :
                                                        asset.assetType === 'lp' ? "bg-amber-500/10 text-amber-500" :
                                                        asset.assetType === 'lending_supply' || asset.assetType === 'lending_borrow' ? "bg-purple-500/10 text-purple-500" :
