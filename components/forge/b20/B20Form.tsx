  ...
  return (
    <div className="animate-in fade-in duration-500">
      <ForgeLayout
        ...
        setIsModalOpen={setIsModalOpen} elementType={elementType} networkName={networkName}
      ...
      >
        ...
        {isAdvancedMode && (
          <div className="bg-background border border-card rounded-xl p-4 flex gap-6 mt-6">
        ...
        <div className={isAdvancedMode ? "grid grid-cols-1 md:grid-cols-3 gap-6 mt-6" : "grid grid-cols-1 md:grid-cols-2 gap-6 mt-6"}>
          <div className={isAdvancedMode ? "md:col-span-2 flex flex-col gap-4" : "contents"}>
            <div>
              <label className="block text-sm font-medium text-secondary mb-2">{b20Type === 'stablecoin' ? 'Stablecoin Name' : 'B20 Asset Name'}</label>
              <input type="text" value={tokenName} onChange={(e) => setTokenName(e.target.value)} className="w-full border border-card rounded-xl p-4 text-foreground focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all" placeholder={b20Type === 'stablecoin' ? 'e.g. Base USD' : 'e.g. Base Native Gold'} />
            </div>
            <div>
              <label className="block text-sm font-medium text-secondary mb-2">Symbol</label>
              <input type="text" value={tokenSymbol} onChange={(e) => setTokenSymbol(e.target.value)} className="w-full border border-card rounded-xl p-4 text-foreground focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all" placeholder={b20Type === 'stablecoin' ? 'e.g. USDb' : 'e.g. BNG'} />
            </div>
            <div className={isAdvancedMode ? "grid grid-cols-1 md:grid-cols-2 gap-4" : "grid grid-cols-1 md:grid-cols-2 gap-6 md:col-span-2"}>
              <div>
                <label className="block text-sm font-medium text-secondary mb-2">Initial Supply Cap</label>
